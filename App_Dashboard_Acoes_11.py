...
def busca_df():
    ...
    #TESTE
    #---------------------
    #Tratando as ações com classe de inspeção em branco
    #Filtra o dataframe carregado do arquivo com as opções selecionadas na barra lateral
        
    #Preenche com espaço vazio as células onde a classe de inspeção está em branco
    df2 = pd.DataFrame
    df2 = df
    df2["Classe da Inspeção"].fillna('', inplace=True)

    #Separa do df as linhas onde as classes de inspeção estão em branco
    df_classe_vazia = df2[df2["Classe da Inspeção"]=='']
    df2 = df2[df2["Classe da Inspeção"]!='']
    
    #Obtendo a lista de subtemas por classe de inspeção
    lista_subtemas = pd.read_excel(
        io = arq_excel_1,
        sheet_name="classe_inspecao",
        engine="openpyxl",
        usecols='A:B',
        nrows = 120
        )
    #Preenche as classes de inspeção em branco, fazendo um procv na lista de subtemas
    df_classe_vazia = df_classe_vazia.merge(lista_subtemas, how='left', on=['Subtema'])
    df_classe_vazia = df_classe_vazia.drop(columns=["Classe da Inspeção_x"])
    
    df_classe_vazia.insert(7, 'Classe da Inspeção', df_classe_vazia.pop('Classe da Inspeção_y'))
 
    #Juntando novamente as duas partes do dataframe
    df2 = pd.concat([df2, df_classe_vazia])
...
#-------------------------
# CRIAÇÃO DE TABELAS DE USO GERAL
#-------------------------
...
def cria_tabelas_auxiliares(df):
    ...
    df["Ano_Data_Limite"]=df["Data limite"].dt.to_period("Y") #Data com tipo datetime
    df["Mes_Ano_String"]=df["Mes_Ano"].astype(str) #converte para string as datas para facilitar os filtros
    df["Ano_Data_Limite_String"]=df["Ano_Data_Limite"].astype(str) #converte para string as datas para facilitar os filtros        
    ...
    #para o e-Fiscaliza já existe no dataframe de origem uma coluna ""Unidade Executante""
    #Para menter compatibilidade, vou renomear a coluna "Unidade Executante" para Unidade_executante
    df.rename(columns={'Unidade Executante': 'Unidade_executante'}, inplace=True)
...
def horas_executadas(df_ent):
    ...
    df_exec["Unidade_executante"]=df_ent["Unidade_executante"]
    #Cria colunas com os meses de execução
    lista_de_meses = periodos(hoje)
...
def cria_tabelas_dos_graficos():    
    ...
    #Filtra do dataframe só os dados dos anos selecionados na barra lateral
    df = df.loc[df["Ano_Data_Limite_String"].isin(fAnos_Limite)]
    ...
    tab1_horas_previstas = tab1_horas_previstas.loc[:, ['Título', 'Situação','Data limite','Classe da Inspeção',"Mes_Ano","Mes_Ano_String","Ano_Data_Limite","Ano_Data_Limite_String","Total de horas"]]
    ...
    #Adicionando coluna com o ano de execução para facilitar a filtragem do ano de interesse
    tab3_cap_horas_mensais_ud["Ano_Execucao"]=pd.to_datetime(tab3_cap_horas_mensais_ud["Mes_execucao"])
    tab3_cap_horas_mensais_ud["Ano_Execucao"]=tab3_cap_horas_mensais_ud["Ano_Execucao"].dt.to_period("Y") #Data com tipo datetime. Pega só o ano
    tab3_cap_horas_mensais_ud["Ano_Execucao_String"]=tab3_cap_horas_mensais_ud["Ano_Execucao"].astype(str) #converte para string as datas para facilitar os filtros
    ...
    tab3_cap_horas_mensais_ud_filtrada =tab3_cap_horas_mensais_ud.loc[(tab3_cap_horas_mensais_ud["Ano_Execucao_String"].isin(fAnos_Limite))&(tab3_cap_horas_mensais_ud["Classe da Inspeção"].isin(fClasseAcao))&(tab3_cap_horas_mensais_ud["Unidade_executante"].isin(fUnidade_Executante))]
...
def tabela_filtro_acoes_grafico(filtro_eventos: pd.DataFrame):
    #Rotina que pega quais a classe de inspeção e mês/ano selecionados
    #no gráfico interativo
    #e mostra, abaixo do gráfico, a respectiva tabela com as ações selecionadas
    #As colunas e demais parâmetros da tabela vêm do que foi selecionado na barra lateral
    #Mostra, abaixo do gráfico, a tabela com as ações selecionadas do gráfico       
    st.text("Classe da Inspeção e Mês/Ano de Vencimento selecionados no gráfico ")
    filtro_eventos
    #Carrega inicialmente todo o dataframe carregado do arquivo excel
    df_filtrado_grafico =  st.session_state["df"]  
    #Faz um primeiro filtro selecionando só as classes de inspeção e ano/mês de vencimento
    #que foram selecionados no gráfico
    df_filtrado_grafico =  df_filtrado_grafico.loc[(df_filtrado_grafico["Classe da Inspeção"].isin(filtro_eventos["Classe da Inspeção"])) &
                                        (df_filtrado_grafico["Mes_Ano_String"].isin(filtro_eventos["Mes_Ano_String"]))]
            
    df_filtrado_grafico.sort_values(by=["Título"],ascending=True)   
            
    #Faz um segundo filtro selecionando com base no que está selecionado na barra lateral
    #quanto à situação das ações e quais as unidades executantes
    df_filtrado_grafico =  df_filtrado_grafico.loc[(df_filtrado_grafico["Situação"].isin(fStatus)) & (df_filtrado_grafico["Unidade_executante"].isin(fUnidade_Executante))]
         
    df_exibido_filtro_grafico = df_filtrado_grafico.filter(items=selecao_colunas)
    st.text(f"{len(df_filtrado_grafico)} Ações Selecionadas Abaixo:")
    #pd.DataFrame(df_filtrado)
    st.dataframe(df_exibido_filtro_grafico)    
...
#-----------------------
#ROTINAS DE CRIAÇÃO DOS GRÁFICOS
#-----------------------

def graf_total_horas_venc_mes(tab_horas_previstas,cor_grafico,altura_grafico,u_execs):

#Cria layout de duas colunas com a primeira 2x mais larga que a segunda
#para motrar uma tabela com as ações selecionadas ao lado do gráfico   
 c1, c2 = st.columns((5, 1))  
 
 with c1:
        
        #c1.header("TOTAL DE HORAS POR MÊS DE VENCIMENTO DAS AÇÕES") 
        #c1.text(f"Unidades Executantes: {u_execs}, Status das ações: {fStatus}") 
        ...
        # Criando uma seleção no gráfico:
        # Qdo seleciona uma barra de uma classe de inspeção
        # a cor de fundo fica cinza, destacando a seleção
        #Define chaves com os tipos de seleção de dados nos gráfcos interativos
        point_selector = alt.selection_point("point_selection")
        multiple_selector = alt.selection_multi(fields=['Classe da Inspeção','Mes_Ano_String'])
        ...
        graf1_vencimento_por_mes = alt.Chart(tab_horas_previstas).mark_bar(
            ...
            color= alt.condition(multiple_selector, "Classe da Inspeção:N", alt.value('lightgray'), legend=alt.Legend(
                   orient='none', legendX=600, legendY=5, direction='horizontal', titleAnchor='middle')), #muda a cor qdo seleciona uma barra
            ...
            ).add_params(multiple_selector).properties(title=titulo1,width=80)
                                            
            #).configure_axis(grid=False).configure_view(strokeWidth=0)
            #Coloca o gráfico na primeira coluna

            #Criando evento qdo seleciona coluna no gráfico
            #No parametro on_select, al clicar no gráfico, é chamada a função principal
            # mostra_dados_filtrados que carrega os gráficos e tabelas com os parametros
            #selecionados na barra lateral. É equivalente a apertar o botão "Enviar" da barra lateral
        event = st.altair_chart(graf1_vencimento_por_mes,use_container_width=False,key="graf1_vencimento_por_mes", on_select=mostra_dados_filtrados, selection_mode=None)
        #Extrai dos eventos os campos de classe de inspeção e de mês e ano
        filtro_eventos = event["selection"]["param_1"]
        #Transforma de dict para dataframe
        filtro_eventos = pd.DataFrame.from_dict(filtro_eventos)
 
 with c2:
        st.text("Selecione") 
        
 #Cria elemento que pode ser ocultado ou exibido mostrando as ações selecionadas
 #a partir da seleção no gráfico
 with st.expander("Ações Selecionadas"):
    if not filtro_eventos.empty:   #Só executa se alguma coluna do gráfico tiver sido selecionada
        #exibe só as colunas selecionadas na barra lateral
        tabela_filtro_acoes_grafico(filtro_eventos)
...
# Gráfico 2 - Quantidade de ações por classe de inspeção, por mês de data limite
    ...
    # a cor de fundo fica cinza, destacando a seleção
    #Define chaves com os tipos de seleção de dados nos gráfcos interativos
    multiple_selector_2 = alt.selection_multi(fields=['Classe da Inspeção','Mes_Ano_String'])
    ...
    graf2_qtd_acoes_por_mes_vencimento = alt.Chart(tab_horas_previstas).mark_bar(
        ...
        column=alt.Column('Mes_Ano_String',title="Ano e Mês de Vencimento das Ações",type="nominal",header=alt.Header(orient='top')),
        color= alt.condition(multiple_selector_2, "Classe da Inspeção", alt.value('lightgray'), legend=alt.Legend(
                   orient='none', legendX=600, legendY=5, direction='horizontal', titleAnchor='middle')), #muda a cor qdo seleciona uma barra
        tooltip=[ alt.Tooltip("Mes_Ano_String", title="Ano/Mês de Vencimento") ,
                alt.Tooltip("sum(Total de horas)",title="Soma do Total de Horas"),"Classe da Inspeção",
                alt.Tooltip("count(Título)",title="Quantidade de Ações")]
    ).properties(height=altura_grafico, width=80, title=titulo2
    ).add_params(multiple_selector_2
    ...
    # mostra_dados_filtrados que carrega os gráficos e tabelas com os parametros
    #selecionados na barra lateral. É equivalente a apertar o botão "Enviar" da barra lateral
    event_2 = st.altair_chart(graf2_qtd_acoes_por_mes_vencimento,use_container_width=False,key="graf2_qtd_acoes_por_mes_vencimento", on_select=mostra_dados_filtrados, selection_mode=None)
    #Extrai dos eventos os campos de classe de inspeção e de mês e ano
    filtro_eventos_2 = event_2["selection"]["param_1"]
    #Transforma de dict para dataframe
    filtro_eventos_2 = pd.DataFrame.from_dict(filtro_eventos_2)
    
    #Cria elemento que pode ser ocultado ou exibido mostrando as ações selecionadas
    #a partir da seleção no gráfico

    with st.expander("Ações Selecionadas"):
        if not filtro_eventos_2.empty:   #Só executa se alguma coluna do gráfico tiver sido selecionada
            #exibe só as colunas selecionadas na barra lateral
            tabela_filtro_acoes_grafico(filtro_eventos_2)
...
#Gráfico comparando a capacidade em horas em cada mês versus a carga de horas de execução estimadas para cada mês
     ...
     #Gráfico da capacidade de horas por mês - Gráfico de Barras
     larg_coluna = 20
     graf_cap_mes = base.mark_bar(
        ...
        color= alt.Color("Classe da Inspeção:N").legend(orient='right').title("Classe da Inspeção"),
     ...
     #Gráfico da carga de horas por mês - Gráfico de bolinhas
     graf_carga_mes = base.mark_circle(opacity=0.99, filled=True,stroke = "black",strokeWidth=2).encode(
        ...
        size="sum(CargaH_Mes)",
        color=alt.Color("Classe da Inspeção:N"),
        tooltip=[ alt.Tooltip("Mes_execucao", title="Ano/Mês de Execução"),  
...
def mostra_dados_filtrados():
    ...
    if  arq_excel_1 != None:
         ...
         st.info(f'''**AÇÕES SELECIONADAS:**  :blue-background[{len(df_exibido)}]  &mdash;     **Mês/Ano da Data Limite das Ações:** {fMesAno}  
                  **Status das ações:** {fStatus}  
                  **Unidades Executantes:** {fUnidade_Executante_exibicao}''')
...
with st.sidebar:
    ...
    if  arq_excel_1 != None:
        ...
        lista_anos = pd.Series(df["Ano_Data_Limite"].unique()).sort_values()
        lista_anos = lista_anos.dropna()  #Remove valores ausentes
        lista_anos_string = lista_anos.astype(str) #converte para string
        ...
        with st.form("Formulário: selecione as opções abaixo e clique em ´Enviar´:"):
            ...
            st.divider()
            st.write("GRÁFICOS:")    
            #Filtro dos anos de vencimento das ações:
            fAnos_Limite =st.multiselect("Anos de Vencimento das Ações:",
                            options=lista_anos_string,
                            default=[ano_atual]        
            )           
            ...
            fUnidade_Executante_exibicao = fUnidade_Executante
            if todas > 0: #Se todas as UD´s nforem selecionadas para exibição dos dados
                fUnidade_Executante = lista_executantes
                fUnidade_Executante.remove("TODAS")
                fUnidade_Executante_exibicao = todas_UDs
            ...
            selecao_colunas = st.multiselect(
                            ...
                            default=["Título","Unidade_executante","Situação","Subtema","Data limite","Descrição","Classe da Inspeção"]
                            )
